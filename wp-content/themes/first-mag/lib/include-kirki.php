		...
		class Kirki_Installer_Control extends WP_Customize_Control {
			...
			public function render_content() {
				...
				<?php if ( ! $installed ) : ?>
					...
					<a class="install-now button-primary button" data-slug="kirki" href="<?php echo esc_url( $plugin_install_url ); ?>" aria-label="Install Kirki Toolkit now" data-name="Kirki Toolkit"><?php esc_html_e( 'Install Now','first-mag' ); ?></a>

					<br/></br><!-- Added <br/> tags to fix the spacing -->

				<?php else : ?>
					<hr>
					<p><?php printf( __( 'The plugin is installed but not activated. Please <a href="%s">activate it</a>.', 'first-mag' ), esc_url_raw( admin_url( 'plugins.php' ) ) ); ?></p>
				<?php endif;
			}
		}
	}
	if ( ! function_exists( 'kirki_installer_register' ) ) {
		...
		function kirki_installer_register( $wp_customize ) {
			...
			$wp_customize->add_section( 'kirki_installer', array(
				'title'       => '',
				'description' => esc_attr__( 'If you want to take full advantage of the options this theme has to provide, please install the Kirki plugin.', 'first-mag' ),
				'priority'    => -10,
